        ...
        for row in ws[signups.ref]:
            ...
            try:
                ...
                if str(row_data["Number Of Showings"]).strip() in ["N/A", "no", ""]:
                    ...
                    # event['preferred_instances'] = 1
                    event["preferred_instances"] = input(
                        f"Number Of Showings for {event['title']} is not a number: {row_data['Number Of Showings']}. Please enter a number: "
                    )
        ...
        for event in events_to_sync:
            ...
            for instance in event["instances"]:
                ...
                db_instance = EventInstance(
                    event=db_event, start=instance["start_time"], end=instance["end_time"], venue=venue
                )
